...
def gauss_seidel_iteration(A, b, tol=1e-6, max_iter=1000):
    ...
    iterations=0
    error = float('inf')
    # DIFFERENCE FROM JACOBI: No separate x_new array needed in Gauss-Seidel    
    ...
    while error>tol and iterations<max_iter:
        ...
        for i in range(n):
            ...
            for j in range(n):
                if i!=j:
                    sum+=A[i][j]*x[j]              
            x[i]=(b[i]-sum)/A[i][i]  #  Direct update to x[i], not x_new[i]
            
            # Updating error as we go through each equation whwereas Jacob calculate error after all variables updated
